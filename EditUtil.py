...
9. Change the internal name of a level (type 9) (Broken)\n""")
...
if choice == "1":
    ...
    levelname = input("Level number:\n")
    newname = input("New internal level name:\n")
    with open("level0"+str(int(levelname)-1)+".cml", "rb") as level_file_handler1:
        cme_level_data = level_file_handler1.read()
        cme_level_name = level_file_handler1.read(51)[23:]
        cme_level_name_end_index = cme_level_name.find(b'\x00\x00\x00\x16\x00')
        # Experimental: level_file_handler1.read(51)[23:].find(b'\x00\x00\x00\x16\x00')
        _newname = '\x00'.join(newname[i] for i in range(0, len(newname))).encode("ASCII")
        newname = _newname
        del(_newname)
        #cme_level_data[23:cme_level_name_end_index] = newname
        cme_level_data[23:cme_level_name_end_index] = newname #TypeError: 'bytes' object does not support item assignment
else:
